...
<div class="container" style="border: 1px solid black;border-radius: 10px;padding: 20px;margin-left: 300px">
    <form action="/product/chatvai" method="post">
        ...
        <div class="form-group">
            ...
            <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" name="thongtin">${item.thongtin}</textarea>
        </div>
        <div class="form-group">
            <label >Hướng Dẫn Bảo Quản :</label>
            <select class="form-control" name="huong_dan_bao_quan.id">
                <c:forEach items="${hdbq}" var="cv">
                    <option ${item.huong_dan_bao_quan.id==cv.id?'selected':''} value="${cv.id}">${cv.ten}</option>
                </c:forEach>
            </select>
        </div>
    ...
    color: black;font-family: 'Lora';text-align: center;">Danh Sách Chất Vải </h1>
        <table class="table" style="margin: 20px 0;">
            ...
            <c:forEach items="${items}" var="sp">
                <tr >
                    ...
                    <th><a href="/product/detail/chatvai/${sp.ma}">Detail</a></th>
                </tr>
            </c:forEach>
            </tbody>
        </table>
